static int tab[100][10];
static char id=0;
static int i,j,k,x=0;
static POINT clic;
static char edition=0;

int mario_present=0;

POINT getkey_x() { //conçu spécialement pour se deplacer dans l'editor avec les fleches gauches et droites
	SDL_Event event;
	while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_KEYDOWN) {
			switch (event.key.keysym.sym) {
				...
				case SDLK_LEFT   : x--; if(x<0)x++;   break;
				case SDLK_RIGHT  : x++; if(x>99)x--;  break;
				default : break;
			}
		}
...
void registrer() { // enregistrer un niveau
	int a=num_molette();
	POINT p1={20,500};
	char chaine[35];
	sprintf(chaine,"les niveaux/niv%d.mario",a);
	level=fopen(chaine,"r");
	...
	if(level==NULL){
		...
		fclose(level);
		aff_pol("un niveau existe deja a ce numero voulez vous le remplacer ? (y/n)",25,p1,white); affiche_all();
		a=-1;
		while(a==-1)a=yes_no();
		if(a){
			level=fopen(chaine,"w+");
			fclose(level);
			level=fopen(chaine,"a");
		}
		if(a==0)level=NULL;
	}
	...
	if(level!=NULL){
		...
		for(i=0;i<80;i++){
			if(t[i]==id_mario)mario_present=1;
		}
		...
		if(mario_present){
			...
			fprintf(level,"%d",t[999]);
		
			aff_pol("votre niveau a bien ete enregistre",30,p1,white);
		}
		else aff_pol("mario est introuvable dans les 8 premieres cases en x, enregistrement annulee",20,p1,white);
		mario_present=0;
		fclose(level);
	}
...
void choix_edition(FILE *level){ //choisir si editer ou non
	char V=-1;
	POINT p1,p2,p3;
	
	p3.x=p3.y=0;
	p1.x=100; p1.y=580;
	aff_pol("voulez vous editer un niveau ? ",20,p1,white);
	
	p1.x=470; p1.y=555; p2.x=545; p2.y=575;
	draw_fill_rectangle(p1,p2,white);
	p1.y=p2.y-2;p1.x+=20;
	aff_pol("Valider",10,p1,black);
	
	
	p1.x=430; p1.y=555; p2.x=450; p2.y=575;
	...
	while(edition==0){
		...
		if(p3.x>=430 && p3.x<=450 && p3.y>=555 && p3.y<=575){
			V=-V;
		}
		
		if(p3.x>=470 && p3.x<=545 && p3.y>=555 && p3.y<=575){
			edition=V;
		}
		
		if(V==1){
			p3.x=434, p3.y=576;
			aff_pol("V",15,p3,white);
		} 
...
void quadrillage(){
	...
	p1.x=0; p2.x=912;
	for(i=0;i<10;i++){
		p1.y=p2.y=i*cases+120;
		draw_line(p1,p2,white);

	}
}

void menu_edition(){ // affiche l'encadré gris et ses boutons
	POINT p1,p2;
	p1.x=0; p2.x=912;
	p1.y=0; p2.y=120;
	draw_fill_rectangle(p1,p2,gris);
	
	p1.x=p1.y=p2.y=20; p2.x=88; 
	draw_paint(background,p1,3); draw_paint(brick,p2,3);
	p1.x=156;
	draw_paint(mariopose,p1,3);
	p1.x=224; p2.y+=48; p2.x=p1.x+48;
	draw_rectangle(p1,p2,white); draw_line(p1,p2,white);
	p1.x=292; p2.x=p1.x+48;
	draw_rectangle(p1,p2,white);
	
}

void choix_clic(){ //pour choisir quoi dessiner si c'est dans l'encadré gris ou placer un bloc si c'est dans le quadrillage
	if(clic.y<120){
		if(clic.y>=20 && clic.y<=68){
			if(clic.x>=20 && clic.x<=68)id=id_background;
			if(clic.x>=88 && clic.x<=136)id=id_brick;
			if(clic.x>=156 && clic.x<=204)id=id_mario;
			if(clic.x>=224 && clic.x<=272)id=0;
			if(clic.x>=292 && clic.x<=340)registrer();
		}
	}
	else {
		if(id==id_mario){
			for(i=0;i<100;i++){
				for(j=0;j<10;j++){
					if(tab[i][j]==id_mario)tab[i][j]=0;
				}
			}
		}
		tab[(clic.x/cases)+x][(clic.y-120)/cases]=id;
	}
}

POINT calcul_pos(int a, int b){ // pour afficher un bloc sur la case choisit en x,y dans le quadrillage
	POINT p;
	p.x=a*cases;
	p.y=b*cases+120;
	return p;
...
void dessine_texture(){ // afficher tous les blocs
	for(i=0;i<19;i++){
		for(j=0;j<10;j++){
			switch(tab[i+x][j]){
				case id_background : draw_paint(background,calcul_pos(i,j),3); break;
				case id_brick : draw_paint(brick,calcul_pos(i,j),3); break;
				case id_mario : draw_paint(mariopose,calcul_pos(i,j),3); break;
				default : break;
			}
		}
	}
...
void mario_editor(){ // le "int main" de ce programme
	init_graphics(912,600); // 19 en x et 10 en y
	SDL_WM_SetCaption("Mario Editor",NULL);
